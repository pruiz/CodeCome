...
class TestPhaseEventLoopEndToEnd:
    ...
    def test_session_snapshot_sync_retries_unresolved_successful_tool_output(self, event_loop_objects, monkeypatch):
        PhaseEventLoop, RunResult, SseClient = event_loop_objects
        emitted: list[dict] = []
        ...
        stale_payload = [{
            ...
            "parts": [{
                ...
                "state": {
                    "status": "completed",
                    "output": "(no output)",
                    "metadata": {"exit": 0, "output": "(no output)"},
                },
            }],
        }]
        final_payload = [{
        ...
        responses = [stale_payload, final_payload]
        ...
        _patch_phase_sse_client(monkeypatch, FakeSseClient)
        monkeypatch.setattr(urllib.request, "urlopen", fake_urlopen)
        monkeypatch.setattr("events.base.time.sleep", lambda _delay: None)

        loop = PhaseEventLoop("http://localhost:8080", "sess-1", None, "1", "recon")
        loop.run(lambda c, p, l, e: emitted.append(e))

        tool_events = [e for e in emitted if e["type"] == "tool_use"]
        assert len(tool_events) == 1
        assert tool_events[0]["part"]["state"]["output"] == "hello\n"
