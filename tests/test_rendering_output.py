...
# ---------------------------------------------------------------------------
# render_questions tests
# ---------------------------------------------------------------------------


class _FakeQuestion:
    def __init__(self, question, why="", affects="", suggested=""):
        self.question = question
        self.why_it_matters = why
        self.affects = affects
        self.suggested_format = suggested


class _FakeQuestions:
    def __init__(self, open_questions=None, rerun_hints=None):
        self.open_questions = open_questions or []
        self.rerun_hints = rerun_hints

    def has_content(self):
        return bool(self.open_questions) or bool(self.rerun_hints)


class TestRenderQuestionsPlain:
    def test_questions_rendered(self, capsys):
        out = RenderOutput(_ctx("plain"))
        qs = _FakeQuestions(
            open_questions=[
                _FakeQuestion("Test Q?", "Important", "Everything", "Text")
            ]
        )
        out.render_questions(qs)
        captured = capsys.readouterr().out
        assert "Open questions" in captured
        assert "Test Q?" in captured
        assert "Why it matters: Important" in captured

    def test_multiple_questions(self, capsys):
        out = RenderOutput(_ctx("plain"))
        qs = _FakeQuestions(
            open_questions=[
                _FakeQuestion("Q1", "reason1"),
                _FakeQuestion("Q2", "reason2"),
            ]
        )
        out.render_questions(qs)
        captured = capsys.readouterr().out
    ...
    def test_no_content_no_output(self, capsys):
        out = RenderOutput(_ctx("plain"))
        qs = _FakeQuestions()
        out.render_questions(qs)
        captured = capsys.readouterr().out
    ...
    def test_hints_rendered(self, capsys):
        out = RenderOutput(_ctx("plain"))
        qs = _FakeQuestions(rerun_hints="Run again with FOO=1")
        out.render_questions(qs)
        captured = capsys.readouterr().out
        assert "Re-run prompt hints" in captured
        assert "FOO=1" in captured

    def test_questions_and_hints(self, capsys):
        out = RenderOutput(_ctx("plain"))
        qs = _FakeQuestions(
            open_questions=[_FakeQuestion("Q?")],
            rerun_hints="Hint text",
        )
        out.render_questions(qs)
        captured = capsys.readouterr().out
        assert "Open questions" in captured
    ...
    def test_missing_fields_not_rendered(self, capsys):
        out = RenderOutput(_ctx("plain"))
        qs = _FakeQuestions(
            open_questions=[_FakeQuestion("Q only")]
        )
        out.render_questions(qs)
        captured = capsys.readouterr().out
        assert "Why it matters" not in captured
        assert "Affects" not in captured


class TestRenderQuestionsRich:
    ...
    def test_hints_rendered(self):
        out = RenderOutput(_ctx("rich"))
        qs = _FakeQuestions(rerun_hints="Run again")
        out.render_questions(qs)
        text = _rich_text_export(out)
        assert "Re-run prompt hints" in text
        assert "Run again" in text


class TestRenderQuestionsTextual:
    def test_calls_fake_sink(self):
        fake = MagicMock()
        fake.write = MagicMock()
        sink = TextualRichLogSink(fake)
        ctx = RenderContext(
        ...
        RenderOutput(ctx).render_questions(qs)
        fake.write.assert_called()

    def test_no_content_no_call(self):
        fake = MagicMock()
        fake.write = MagicMock()
        sink = TextualRichLogSink(fake)
        ctx = RenderContext(
