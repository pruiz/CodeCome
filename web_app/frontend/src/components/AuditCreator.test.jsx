...
describe('AuditCreator', () => {
  ...
  it('includes selected question owner in create payload', async () => {
    ...
    await user.click(screen.getByRole('switch', { name: /Run Phase 1 enrichment before phase-2/i }));
    ...
    await waitFor(() => {
      ...
      expect(JSON.parse(createCall[1].body)).toMatchObject({
        ...
        auto_continue: true,
        model_settings: { __audit_options: { worker_model: 'local/qwen3.6-27b', run_phase1_enrichment_auto: true, run_gap_scan_auto: true } },
      });
    });
  });
  ...
  it('passes selected zip file and settings to upload API', async () => {
    ...
    await waitFor(() => {
      ...
      expect(JSON.parse(uploadCall[1].body.get('model_settings'))).toEqual({ __audit_options: { worker_model: 'local/qwen3.6-27b', run_phase1_enrichment_auto: true, run_gap_scan_auto: true } });
    });
  });
});
