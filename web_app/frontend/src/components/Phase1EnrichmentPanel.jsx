...
export default function Phase1EnrichmentPanel({ audit, onRefresh }) {
  ...
  const savePrompt = async (nextPrompt) => {
    ...
    try {
      ...
      await onRefresh?.();
      setMessage(String(nextPrompt || '').trim() ? 'Phase 1 enrichment prompt saved.' : 'Phase 1 enrichment prompt reset to Preview Analysis default.');
    } catch (err) {
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-950 p-4">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="font-mono text-xs text-gray-500">{prompt?.custom ? 'Audit custom prompt' : (prompt?.path || 'Preview Analysis prompt')}</div>
          </div>
          <div className="flex flex-wrap gap-2">
        ...
        <textarea
          ...
          spellCheck={false}
          placeholder="Preview Analysis prompt or audit-specific enrichment prompt will appear here."
